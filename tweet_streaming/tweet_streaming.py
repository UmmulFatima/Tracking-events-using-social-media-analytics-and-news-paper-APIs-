...
if __name__ == '__main__':
    try:
        ...
        fhOut = codecs.open('Data/' + file_name, "w+", "utf-8")
        ...
        with open("tweet_ids_only.txt", "r") as myfile:
            for line in myfile:
                api = tweepy.API(auth, parser=tweepy.parsers.JSONParser())
                tweet_object = api.get_status(line)
                tweet = json.dumps(tweet_object)
                print(tweet)
                tweet_date = tweet.created_at
                tweet_text = tweet.text
                print(tweet_text,tweet_date)
